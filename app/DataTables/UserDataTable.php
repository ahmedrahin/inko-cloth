...
class UserDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('order', function (User $user) {
                $count = $user->orders->count();
                $colorClass = $count > 0 ? 'badge bg-primary' : 'badge bg-danger';
                return '<span class="text-white ' . $colorClass . '">' . $count . '</span>';
            })
            ->addColumn('buying', function (User $user) {
                $totalSpent = $user->orders->sum('grand_total');

                // Convert large numbers to K, M format
                if ($totalSpent >= 1000000) {
                    $formattedTotal = number_format($totalSpent / 1000000, 1) . 'M';
                } elseif ($totalSpent >= 1000) {
                    $formattedTotal = number_format($totalSpent / 1000, 1) . 'K';
                } else {
            ...
            ->setRowId('id')
            ->rawColumns(['role', 'last_login_at', 'status', 'id', 'order', 'buying', 'action']);
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
            ...
            ->dom(
                ...
                "<'row align-items-center'<'col-sm-12 col-md-6 d-flex gap-3'l i><'col-sm-12 col-md-6 d-flex justify-content-end'p>>"
            )
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('order')->title('Total Order')->addClass('text-center'),
            Column::computed('buying')->title('Total Buying')->addClass('text-center'),
